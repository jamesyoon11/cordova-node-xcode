...
pbxProject.prototype.addXCConfigurationList = function(configurationObjectsArray, defaultConfigurationName, comment) {
    ...
    var newPbxBuildConfigurationSection = {};
    var restPbxBuildConfigurationSection = {};
    var count = 0;
    for (var property in pbxBuildConfigurationSection) {
        if (count++ < 4) {
            newPbxBuildConfigurationSection[property] = pbxBuildConfigurationSection[property];
        } else {
            restPbxBuildConfigurationSection[property] = pbxBuildConfigurationSection[property];
        }
    }
    for (var index = 0; index < configurationObjectsArray.length; index++) {
        ...
        newPbxBuildConfigurationSection[configurationUuid] = configuration;
        newPbxBuildConfigurationSection[configurationCommentKey] = configuration.name;
        xcConfigurationList.buildConfigurations.push({ value: configurationUuid, comment: configuration.name });
    }
    for (var property in restPbxBuildConfigurationSection) {
        newPbxBuildConfigurationSection[property] = restPbxBuildConfigurationSection[property];
    }
    this.hash.project.objects['XCBuildConfiguration'] = newPbxBuildConfigurationSection;
    ...
    var buildConfigurationsList = [
        {
            ...
            buildSettings: {
                ...
                SKIP_INSTALL: 'YES',
                CODE_SIGN_ENTITLEMENTS: subfolder + '/' + name + '-Debug.entitlements',
                CODE_SIGN_IDENTITY: '$EX_SIGNING_IDENTITY',
                PROVISIONING_PROFILE_SPECIFIER: '$EX_PROVISION',
                DEVELOPMENT_TEAM: '$EX_DEVELOPMENT_TEAM',
        ...
        },
        {
            ...
            buildSettings: {
                ...
                PRODUCT_NAME: '"' + targetName + '"',
                SKIP_INSTALL: 'YES',
                CODE_SIGN_ENTITLEMENTS: subfolder + '/' + name  + '-Release.entitlements',
                CODE_SIGN_IDENTITY: '$EX_SIGNING_IDENTITY',
                PROVISIONING_PROFILE_SPECIFIER: '$EX_PROVISION',
                DEVELOPMENT_TEAM: '$EX_DEVELOPMENT_TEAM',
                PRODUCT_BUNDLE_IDENTIFIER: bundleId
